...
public class Code128 extends Code128Constants
{
    ...
    public Code128(String text) throws DecodeException, CharNotInCodesetException
    {
        String encoded = null;
        ...
        {
            throw new DecodeException("'" + text + "' <> '" + encoded + "'");
        }
    }
    ...
    public static char[] encode(String txt) throws CharNotInCodesetException
    {
        int codeset;	/*
         * Code 0 = A ,...
         */
    ...
    public static char codeIn(char c, int codeset) throws CharNotInCodesetException
    {
        char i;
        ...
        throw new CharNotInCodesetException("char " + c + " not in codeset " + codeset);
    }

    public static String decode(char[] bars) throws DecodeException
    {
        char ii, jj, kk;
        char bar;
        char space;
        ...
        for (ii = 0; ii <= bars.length - 7; ii += 6)
        {
            ...
            if (odd(bar))
            {
                throw new DecodeException("ii=" + ii + " bar(" + bar + ") is not odd!");
            }
            if (even(space))
            {
                throw new DecodeException("ii=" + ii + " space(" + space + ") is not even!");
            }
            /*
             * Decode it
             */
            ...
            {
                throw new DecodeException("decoding failed ii=" + (int) ii);
            }
            S = tabDecode[cDecoded][4];
            if (bar != S)
            {
                throw new DecodeException("ii=" + ii + " Bar checksum failed!");
            }
            if (!((S - 1.75) < (b1 + b2 + b3) && (b1 + b2 + b3) < (S + 1.75)))
            {
                throw new DecodeException("ii=" + ii + " One module edge error!");
            }

            if (cDecoded > 107)
            {
                throw new DecodeException("Decoded > 107");
            }
            sb.append(cDecoded);
        }
        ...
        {
            throw new DecodeException("Symbol checksum of '" + sb.toString() + "' failed! sum = " + (int) sb.charAt(sb.length() - 2) + " should be " + iChkSum % 103);
        }
                ...
                case CODEA:
                    switch (cDecoded)
                    {
                        ...
                        case 106:	/*
                            ...
                            {
                                throw new DecodeException("Check Sum failed");
                            }
                        case 107:	/*
                             * STOP BACKWARDS
                             */
                            throw new DecodeException("STOP BACKWARDS");
                        default:
                            if (bAdd128 == true)
                            {
                ...
                case CODEC:
                    switch (cDecoded)
                    {
                        ...
                        default:
                            if (cDecoded > 99)
                            {
                                throw new DecodeException("Decoded > 99");
                            }
